...
- [ ] HIGH: Mail-sending endpoints authorize by Origin header and rate limit only, both bypassable by a non-browser caller | Why: notify-application.ts, notify-mixer-rsvp.ts, verify-turnstile.ts and alert-failure.ts all gate their POST handler on isAllowedOrigin(origin) plus enforceRateLimit alone; Origin is a client-supplied header any direct HTTP caller can set to whatever it wants, and enforceRateLimit fails open (returns null) whenever UPSTASH_REDIS_REST_URL/TOKEN aren't configured (src/lib/rateLimit.ts:134-136), so a forged request can trigger emails to NOTIFICATION_EMAIL or ops pages with attacker-chosen name/email content, rate-limited only if Upstash happens to be configured in that environment. CodeRabbit flagged this against the new notify-mixer-rsvp.ts on PR #265, but the pattern is pre-existing and shared by notify-application.ts (already on main, predates this session), so patching only the new file would leave 3 production endpoints inconsistent with it, which this codebase's consistency rule forbids. None of the 4 endpoints write Firestore directly (the actual lead write goes through the separately validated capture-lead.ts), so today's exploit ceiling is inbox/alert spam, not data corruption. | Files: src/pages/api/notify-application.ts, src/pages/api/notify-mixer-rsvp.ts, src/pages/api/verify-turnstile.ts, src/pages/api/alert-failure.ts, src/lib/allowedOrigin.ts, src/lib/rateLimit.ts | Plan: give each endpoint a server-side credential to check instead of trusting Origin (e.g. a short-lived token the endpoint that already validated the real payload issues, like capture-lead.ts for the lead/notify pair, or the Turnstile-verified token itself for verify-turnstile); audit each endpoint's actual write exposure first since severity differs per endpoint | Verify: a request with a forged Origin header and no valid credential is rejected on all 4 endpoints; the real client flows (apply form, mixer RSVP, turnstile verification, ops alerts) still succeed end to end
...
- [ ] HIGH: Weekly Smoke Tests workflow on main has failed 6 consecutive scheduled runs (Jul 22 through Aug 26 2026, latest run 32962540856), every failure on the iphone Playwright project: apply form success panel never appears, HomeSignup and both waiver flows fail, every city page test fails | Why: a permanently red scheduled job means real mobile regressions land unseen, and 70% of traffic is mobile; the waiver half of this is now explained and fixed (the `apply-terms` selector rot on `/waiver`, corrected 2026-09-11), but the apply/city/signup failures are broader than that and remain unclassified | Files: tests/smoke/critical-flows.spec.ts, tests/smoke/site.spec.ts, playwright.config.ts | Plan: run the iphone project locally against a build to classify each failure as selector rot, mobile overlay intercepting taps or a real regression; fix test-side rot directly, split any real mobile UI regression into its own entry with the owning component files | Verify: smoke workflow green on main for the iphone project on the next scheduled run plus 3 consecutive green local runs of both spec files under the iphone project
...
- HIGH: [RECOVERY-ORDERING] `src/lib/eventbriteRecovery.ts:135-138` checks `event.defaultPrevented` before later listeners or ancestor listeners execute, so a suppressing handler further along the bubble path leaves the checkout CTA dead.
  Decision (2026-09-11): obsolete, nothing left to reorder. `src/lib/eventbriteRecovery.ts`, `src/lib/eventbriteRecovery.test.ts` and `EventbriteWidgetInit.astro` no longer exist anywhere in `src/`; all three were deleted with the embedded-checkout removal (PR #196), which replaced the widget with a plain redirect that has no `defaultPrevented` guard. Kept so the report is not re-filed.
- HIGH: [SCOPE-CREEP] `LESSONS.md:9` turns Eventbrite-specific timing and DOM behavior into constraints for “Any integration,” while `src/lib/navigation.ts:7-9` mandates the wrapper for all navigation, conflicting with direct navigation at `src/layouts/BaseLayout.astro:347`.
  Decision (2026-09-11): half stale, half fixed. The navigation half is gone on its own: `src/lib/navigation.ts` was removed with the embedded-checkout path (PR #196), so the universal wrapper mandate and its claimed conflict with `BaseLayout.astro:347` no longer exist. The LESSONS.md half was real and survived at line 89 (it had moved from line 9), so it was fixed rather than closed: the rule now keeps the general requirement (a `preventDefault()` override needs a verification check plus a native-fallback recovery) and marks the modal-observation and same-tab-navigation details as Eventbrite specifics to reason about per integration instead of constraints on "any integration". See the 2026-09-11 CHANGELOG entry. Kept so the report is not re-filed.
