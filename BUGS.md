...
- [ ] CRITICAL: `ticketSource: "eventbrite-owned"` is set on 13 shows in events.ts but the business does not currently control any Eventbrite organizer account, so Purchase tracking to Meta (src/pages/api/sync-orders.ts) has likely never worked correctly and the daily sync cron is either silently finding nothing or (worse) silently succeeding against the wrong account's data | Why: user stated directly (2026-08-27) "PHILY IS NOT OUR EVENTBRITE ACCOUYNT NOTHING IS OUR EVENTBRITE ANY MORE" after I incorrectly told her Philadelphia purchase tracking works because it's "eventbrite-owned"; per the field's own doc comment in events.ts (line 24-37) this flag is manually set business knowledge, not derivable from code, so the data itself is stale, not a code bug; open question for the user: does the business currently control ANY Eventbrite organizer account (even a new/different one), or is ticketing fully off Eventbrite now | Files: src/data/events.ts, src/pages/api/sync-orders.ts, src/lib/eventbrite.ts | Plan: get the user's answer on current Eventbrite account ownership first; if a valid organizer account exists, verify EVENTBRITE_API_TOKEN in Vercel matches it and correct ticketSource per-event to match reality; if no account exists, flip all 13 "eventbrite-owned" events to "external" and treat server-side Purchase tracking as unavailable until a replacement mechanism exists | Verify: for a real known-purchased Eventbrite ticket order, confirm GET requests using the configured EVENTBRITE_API_TOKEN against that event's orders endpoint return real matching order data (not 401/403/empty), then confirm CHANGELOG.md records which events were relabeled
