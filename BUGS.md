...
- HIGH: [SCOPE-CREEP] `LESSONS.md:9` turns Eventbrite-specific timing and DOM behavior into constraints for “Any integration,” while `src/lib/navigation.ts:7-9` mandates the wrapper for all navigation, conflicting with direct navigation at `src/layouts/BaseLayout.astro:347`.
  Decision (2026-09-11): not fixable as filed, both halves are stale. `src/lib/navigation.ts` was removed with the embedded-checkout path (PR #196), so the universal wrapper mandate and its claimed conflict with `BaseLayout.astro:347` are already gone. The remaining half is a single surviving LESSONS.md line, which has since moved from line 9 to line 89; narrowing that lesson's wording is a LESSONS.md editorial call for the owner, not an atomic bug fix, and is not carried out here on a reviewer's reading of line numbers that no longer match. Kept so the report is not re-filed.
