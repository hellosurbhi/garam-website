...
- [ ] HIGH: One permanently-failing syncable event can silently stall Purchase tracking for every other show, forever, with no alert | Why: sync-orders.ts holds its shared `changed_since` cursor (`lastSyncAt`) at its previous value whenever ANY syncable event's fetch fails this run (intentional, to avoid silently skipping a transient miss on that one event), but nothing distinguishes a transient blip from a permanent one (wrong ticketSource, revoked token, deleted Eventbrite event); a single misconfigured currently-relevant event would jam Purchase syncing for every other event too, and hadSyncFailure never calls alertOps, only a fully uncaught exception does, so this fails silently | Files: src/pages/api/sync-orders.ts | Plan: track consecutive failure counts per eventbriteId in sync metadata; after N consecutive failures for one event, exclude only that event from cursor-advancement gating (keep retrying it independently) while letting the global cursor advance for events that succeeded, and alertOps once when an event crosses the threshold | Verify: emulator/unit test simulating one permanently-failing event alongside N-1 succeeding events confirms the cursor still advances for the successful events after the threshold, and alertOps fires exactly once at the crossing. A same-session mitigation for the specific past-show manifestation of this (2026-08-27, commit pending): events more than 3 days past their show date now drop out of the syncable set regardless of ticketSource, so old/uncertain-history shows can no longer trigger this; the general design gap for currently-relevant events remains open.

- [ ] MEDIUM: Upstash Redis rate-limit check times out on every real production request (2/2 observed), so click-spam protection on /api/go/[slug] is currently non-functional | Why: production logs right after the 2026-08-27 tracked-redirect fix deploy show `[rateLimit] limiter error, failing open: Error: Timeout: Upstash rate limit check (1500ms)` on test clicks for the philadelphia-2026-08-28 and washington-dc-2026-08-30 event pages; design intent is fail-open so checkout/tracking still work correctly (confirmed, not a user-facing bug), but the rate limiter itself appears to always be losing the race against its own 1500ms ceiling, meaning spam-click abuse protection is silently disabled | Files: src/lib/rateLimit.ts | Plan: check whether UPSTASH_REDIS_REST_URL points at a region far from Vercel's iad1, confirm the Upstash database is actually reachable/not paused, and consider whether 1500ms is too aggressive for actual observed latency vs raising it or diagnosing the real network cause; also wire the outer timeout's AbortSignal into the Upstash Redis client so a timed-out request is actually cancelled instead of continuing to run in the background, and give `enforceRateLimit` a distinct "unavailable" return value (not just true/false) so callers can suppress the CAPI call specifically when the limiter couldn't answer, instead of treating "timed out" the same as "confirmed not spam" | Verify: trigger a real /api/go/[slug] request and confirm no "[rateLimit] limiter error" line appears in Vercel function logs, or if latency is inherently higher than 1500ms, confirm a revised timeout no longer times out on 5 consecutive real requests; add a test asserting a timed-out limiter call suppresses the CAPI send rather than allowing it through

- [ ] MEDIUM: Coverage gate credits default-parameter expressions as "covered" whenever their enclosing function was called, even on the code path where the default never runs | Why: `scripts/diff-coverage.mjs`'s function-hit rescue marks every statement between a function's declaration and its body as covered as soon as the function itself shows any hits; a parameter default like `function f(x = compute())` is a conditionally-executed branch (skipped whenever a caller passes `x` explicitly), not always-covered syntax, so a change that only breaks the default-value path can pass the patch-coverage gate with zero real coverage of that path | Files: scripts/diff-coverage.mjs | Plan: use the v8/Istanbul `branchMap` (which already tracks `AssignmentPattern` defaults) instead of the function-hit rescue for parameter-default lines, so only actually-syntax-only lines (multi-line signatures, closing parens) get the rescue and executable defaults are checked against real branch coverage | Verify: a test fixture with a function whose default-parameter path is never exercised by its test should fail the gate at 0% for that line; once a test exercises both the supplied-argument and default-argument paths, the gate passes
