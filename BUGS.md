...
- [ ] MEDIUM: [DATA-MIGRATION-MISSING] The Purchase-CAPI retry query only matches orders where both new booleans are explicitly false | Why: order documents created before PR #196 have neither `purchaseCapiSent` field, Firestore equality filters skip documents missing the field, so pre-existing undelivered orders can never be retried without a backfill | Files: src/pages/api/sync-orders.ts | Plan: one-time backfill setting the booleans to false on documents missing them (a not-equal-true query is NOT a substitute: Firestore inequality filters also exclude documents where the field is absent, so only an explicit backfill reaches fieldless documents) | Verify: emulator test with a fieldless order document confirms it enters the retry set. Executor: the 3:00 overnight fix-medium-bugs lane.
