...
- WONTFIX(LOW, 2026-09-11): [POSTHOG-KEY-IN-TRANSCRIPT] The PostHog project key `phc_...` from `public/js/posthog.js:124` appears in old Claude chat transcripts on disk | Reason: phc_ project keys are public by design; the site ships this key in page JavaScript to every visitor, so a transcript copy adds zero exposure beyond what production already publishes. PostHog's own docs class it as safe to embed client-side (write-only event capture, no data read access). Rotating it would cost a settings reset plus code change plus redeploy and the new key would be public again on the next page load. The genuinely secret credential leaked alongside it (Upstash REST token) was rotated at the provider on 2026-09-11 and both Vercel envs updated. Recorded so the report is not re-filed.
...
- [ ] HIGH: [STALE-SPONSOR-CONTACT] The sponsorship pages still tell sponsors to email Caleb, who no longer works with the show, so that contact path is broken today | Why: flagged-to-close PR #123 removed the stale name and email but never merged; main's src/data/sponsorship.ts still says "Email Caleb, our manager..." and src/pages/sponsorship.astro renders it | Files: src/data/sponsorship.ts, src/pages/sponsorship.astro | Plan: recreate only the contact-info slice of PR #123 on fresh main: replace the stale person with the current sponsorship contact route | Verify: grep shows no Caleb reference in src/, the sponsorship page renders the current contact, build passes.
- [ ] MEDIUM: [DUAL-ADMIN-AUTH] Admin access is checked two different ways with two different lists, so the dashboard breaks again whenever the hidden env-var list drifts | Why: flagged-to-close PR #145 unified auth on one email allowlist but never merged; src/lib/verifyToken.ts still authorizes via the ADMIN_UIDS env var (lines ~86, ~136) while newer paths use email claims | Files: src/lib/verifyToken.ts, src/lib/adminAllowlist.ts, scripts/verify-admin-emails.mjs | Plan: recreate PR #145 on fresh main: one email allowlist as the single source of truth for every admin check, carrying over the old PR's operator instructions | Verify: admin API tests pass with the allowlist as the only grant path and an out-of-list email is rejected everywhere.
- [ ] HIGH: [PREFETCH-FAKE-CONVERSIONS] Browser prefetches of the ticket link are counted as Meta InitiateCheckout conversions, feeding the ad targeting bad data and wasting ad spend | Why: flagged-to-close PR #239 fixed this (isSpeculativeRequest.ts, click-time eid stamping, rate-limit ordering) but never merged; main has no Sec-Purpose/prefetch handling anywhere under src/pages/api | Files: src/lib/isSpeculativeRequest.ts, src/pages/api/ticket-click.ts, test/ticketCtaTracking.test.ts | Plan: recreate PR #239 on fresh main including its tests: detect speculative requests via Sec-Purpose/Purpose headers and stamp conversion events at click time only | Verify: test/ticketCtaTracking.test.ts passes; a simulated prefetch request produces no conversion event while a real click does.
