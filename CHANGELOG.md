...
- **A second Codex round found the redaction pattern was still narrower than the validator, on the other half.** The first fix widened the local part to match everything `validateEmail` accepts and said so in a comment, but the domain half was rewritten at the same time into `[^\s@.]+(?:\.[^\s@.]+)+`, which requires exactly one dot between labels. The validator's `[^\s@]+\.[^\s@]+` puts no such limit anywhere but the first and last character, so `priya@exa..mple.com` and `priya@.example.com` are both accepted at the apply form and matched by NEITHER pattern, which is not a partial redaction but the whole address published to the public topic. The domain is now taken as a whole run behind a lookahead that asserts the dot (`[^\s@]+@(?=[^\s@.]*\.)[^\s@]+`), so it is a superset of the validator by construction instead of by a second hand-written spelling of the same grammar; the lookahead scans over a dot-excluding class, keeping the two halves of the pattern from matching the same characters, which is the backtracking property the inline form was reaching for. `test/ops-alert.test.ts` now asserts the coverage relation itself: every address in the table is first asserted to pass `validateEmail`, then asserted to leave no `@` in the webhook body, so a future loosening of the front door that outruns the redactor fails the suite instead of shipping. A bare `@garammasaladating` handle is still left alone, since an address needs a local part.
