...
- A Codex round on this same work filed three HIGH findings. Two are folded in above (the local-part gap in the redaction pattern, and the overclaiming that treated a documented required check as a live one). The third, that requiring every `ci.yml` job and banning job-level `if:` is a repo-wide policy beyond the finding's scope, is recorded as a won't-fix decision in `BUGS.md`: that policy is the owner's, is written in the `ci.yml` header since 2026-07-14, and the test asserts it in the two files that can drift from it rather than inventing it.
