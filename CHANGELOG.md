...
## fix(sync): stop syncing already-completed shows so a stale ticketSource label can never jam Purchase tracking (2026-08-27)

- `src/pages/api/sync-orders.ts`: the daily sync held its shared `changed_since` cursor at its previous value whenever ANY syncable event's Eventbrite fetch failed this run, with no distinction between a transient blip and a permanent one (e.g. a show wrongly, or no longer accurately, marked `ticketSource: "eventbrite-owned"`). One old show we don't actually have order access to could silently stall Purchase syncing for every other show, including ones that matter, forever, with no alert. Added a 3-day trailing window: events more than 3 days past their show date now drop out of the syncable set regardless of `ticketSource`, since a show that old has no realistic new orders left to sync anyway.
- This makes the exact `ticketSource` value on an already-completed show functionally inert: whether it says `"eventbrite-owned"` or `"external"`, the sync job will never touch it once it's a few days old, so there is no longer any live reason to determine which past shows really were run through a business-owned Eventbrite account and which weren't.
- Queued the deeper, still-open design gap in BUGS.md (HIGH): the shared-cursor stall can still happen for a currently-relevant event, not just an old one; that needs a per-event permanent-failure circuit breaker, a bigger change than this session's scope.
- Verified: `npx astro check` (0 errors), `npx vitest run` (1241/1241 passing). No existing test coverage exists for `sync-orders.ts` (pre-existing, tracked gap, see BUGS.md MISSING-COVERAGE).

**Files:** `src/pages/api/sync-orders.ts`, `BUGS.md`
...
- A first pass at this fix wrongly flipped 9 already-completed past shows (Apr 19 through Aug 16 2026) to `"external"` too. Reverted those 9 back to `ticketSource: "eventbrite-owned"`, their pre-existing value, since the correction the business gave was about current/going-forward account access, not a confirmed fact about each individual past show. See the next changelog entry: the exact label on a completed show no longer has any live effect either way.
