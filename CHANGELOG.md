...
## content(emails): move every email template's copy out of emails.ts into its own JSON file (2026-08-27)

- `src/data/emails.ts` held every applicant/contestant-facing email's literal English wording inline in template-literal and array-join form, so any unrelated code change to the file risked accidentally rewording a live email. Extended the pattern already shipped for `applicationReceived` (`src/data/email-copy/applicant-welcome.json`) to the remaining 10 template functions: each now imports a sibling `.json` file under `src/data/email-copy/` holding `{subject, text, html}` with `{{token}}` placeholders and a `_note` field marking it do-not-edit without exact word-for-word replacement copy.
- `inviteApproval`'s two independent conditionals (show-details line, portal-link line) each store both wordings as named variants (`showLineVariants`, `portalLineVariants`) in JSON; `emails.ts` only selects which variant applies, it never invents wording. `hostBriefing` (an internal ops digest, not applicant-facing) keeps its data-driven `<table>` composed at runtime and only pulls its two wrapper lines from JSON. `newShowAnnouncement`'s caller-supplied `subject`/`customMessage` stay runtime parameters, never copy; only its static fallback line and wrapper copy moved to JSON.
- Replaced chained `.replaceAll()` substitution with a single-pass `fillTemplate()` helper (one regex `.replace()` over the original template string) for every newly extracted function, so a dynamic value that happens to contain another token's literal text (an applicant's name, or the freeform signed waiver text) can no longer get re-substituted by a later `.replaceAll()` call scanning already-substituted output.
- Every existing security transform is preserved exactly: `subjectSafe()` before every subject substitution, `escapeHtml()` before every HTML substitution, raw values in `text`, and `link()`'s http(s)-only scheme allowlist for every URL.
- Verified with `npm run check` (0 errors), full `npx vitest run` (1228/1228 passing), and a before/after render diff of all 10 functions (both `inviteApproval` conditional branches, both `hostBriefing` singular/plural cases, and an adversarial waiver-text sample containing `<b>`, quotes, an ampersand and a literal `{{token}}` string) confirming byte-for-byte identical output.
