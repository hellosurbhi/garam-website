...
- Tests: `test/go-redirect.test.ts` covers every speculation header plus the real top level navigation that must still fire CAPI; new `test/ticketCtaTracking.test.ts` covers the click-time stamping, the wire-time href staying free of `eid`, single-wire idempotency, and the new-tab and same-tab click paths; new `test/isSpeculativeRequest.test.ts` and `test/isBotUserAgent.test.ts` pin the two filters directly, including the cases the redirect test cannot reach (bare `prerender`, the legacy `preview` spellings, a `Purpose` header that announces no speculation, and every unfurl bot on the denylist against four real browser User-Agents that must never be filtered). That also narrows the standing MISSING-COVERAGE backlog entries in BUGS.md.
