...
## fix(tracking): browser prefetches no longer count as checkout conversions (2026-08-26)

Closes the Codex 2026-08-03 [UNRESOLVED-BACKLOG] finding, which correctly said the earlier client-side `eid` work left the hole open.

- `/api/go/[slug]` fired a Meta CAPI `InitiateCheckout` for browser prefetches. A prefetch is issued by the visitor's own browser (Chrome's "preload pages" setting, speculation rules, `<link rel=prefetch>`, extension prefetchers), so it carries an ordinary browser User-Agent, the visitor's cookies and their IP, and sailed straight through `src/lib/isBotUserAgent.ts`, which can only recognize fetches made by someone else's server. Each one was reported as a click that never happened. Since Meta uses conversions as the training signal for finding more people like whoever converted, that noise degrades ad targeting rather than merely inflating a number.
- New `src/lib/isSpeculativeRequest.ts` recognizes them by the headers browsers send for exactly this purpose: `Sec-Purpose` (the current standard, covering `prefetch` and `prefetch;prerender`), its legacy `Purpose` spelling, Safari's `X-Purpose`, Firefox's `X-moz`, and Fetch Metadata (a `Sec-Fetch-Mode` that is present and not `navigate`, since the only human path to a tracked redirect is a top level navigation). Requests with no `Sec-Fetch-Mode` at all, meaning non-browser clients, are still left to the User-Agent denylist. Speculative requests keep getting the honest 302, since answering them with anything else would break the navigation if the visitor does go on to click.
- `src/lib/ticketCtaTracking.ts` now stamps the `?eid=` dedup id onto the href at click time instead of at wire time. An href that already carries the id is itself prefetchable, and stamping at click time also means the URL a click navigates to is never the URL a prefetcher already cached, so a prefetch cache hit cannot swallow the click and cost us the conversion. UTM forwarding stays at wire time, since it is prefetch-safe.
- The server keeps generating a fallback `eid` when a request arrives without one (no-JS fallback, shared or bookmarked link): a missing `eid` was never what separated a click from a non-click, and treating it that way would drop real conversions. The speculation headers do that job.

Three follow-up fixes from the Codex review of the work above (2026-08-26), all in the same area:

- The per-IP rate limit was being spent by requests that could never fire CAPI. `enforceRateLimit` consumes one unit of the shared 30 per minute budget on every call, and it ran before the bot and speculation checks, while a prefetch arrives on the visitor's own IP. A page speculating over several ticket links could therefore burn the whole budget on fetches that were going to be filtered anyway, and the visitor's real click moments later found it empty and went untracked: the guard meant to protect the Meta spend was suppressing the exact conversions it exists to preserve. The eligibility checks now run first and the budget is only charged to a request that has earned a Meta call.
- Every response from `/api/go/[slug]` is now `Cache-Control: no-store`. Filtering a prefetch out of CAPI was only half the job: the prefetched 302 still landed in the browser cache, and a cached 302 can satisfy the visitor's real activation without the request ever reaching us, so the conversion the prefetch was correctly denied was never fired by the click either. That loss is invisible from the server, because the click simply never arrives. Making the redirect non-storable means a speculative fetch can never stand in for a real one, whichever way the visitor triggers it, including the paths no client-side handler can see: context-menu "Open link in new tab", dragging the link to a tab bar, and no-JS navigation. Nothing here was worth caching anyway, since it is a 302 whose only job is firing one tracking event.
- Middle-clicks now track. `click` only fires for the primary button, so the handler's existing `button !== 0` branch was unreachable for the one case it was written for: a middle-click fires `auxclick`, and every browser treats it as "open this link in a new tab". Those reached checkout with no `checkout_opened` event and no `eid` on the href. `click` and `auxclick` now share one handler and one per-anchor id. Button 2 is deliberately ignored, because the right button opens a context menu rather than navigating, and counting it would report checkout intents for menus the visitor dismisses; a tab actually opened from that menu is covered by the no-store response instead. The review also listed shared links here, but those are already correct: a link opened on someone else's device has no prefetch cache to hit, and the server mints its own `eid` for it.

- Tests: `test/go-redirect.test.ts` covers every speculation header plus the real top level navigation that must still fire CAPI, that prefetches, unfurl bots and an unconfigured CAPI token never touch the rate limit while a real navigation still does, and that the checkout redirect, the `/tickets` fallback and the 404 all carry `no-store`; new `test/ticketCtaTracking.test.ts` covers the click-time stamping, the wire-time href staying free of `eid`, single-wire idempotency, the new-tab and same-tab click paths, and the middle-click `auxclick` path including id reuse across a click and a middle-click, the untouched native navigation, and the ignored right button; new `test/isSpeculativeRequest.test.ts` and `test/isBotUserAgent.test.ts` pin the two filters directly, including the cases the redirect test cannot reach (bare `prerender`, the legacy `preview` spellings, a `Purpose` header that announces no speculation, and every unfurl bot on the denylist against four real browser User-Agents that must never be filtered). That also narrows the standing MISSING-COVERAGE backlog entries in BUGS.md.
