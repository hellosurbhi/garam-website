...
- [ ] ENHANCEMENT: keep the alert dedupe Redis database alive so Upstash does not delete it for inactivity again | Why: the `garam-alert-dedupe` Upstash Redis database (env vars in PR #244 operator step 2) is only touched during an actual alert incident, which is rare; Upstash's free tier already deleted one such database for inactivity on 2026-09-09, forcing a manual recreate of the database and re-wiring of both Vercel and `.env.local`. The form itself was never at risk (`alert-failure.ts`'s claimAlert() fails open without these envs, meaning worst case is a duplicate alert email, not a broken form), but losing the database silently defeats the dedupe feature until someone notices | Files: src/pages/api/alert-failure.ts, .github/workflows (a new scheduled workflow) | Plan: add a lightweight scheduled GitHub Action that pings the Upstash REST endpoint with a harmless read/write (a heartbeat key with a short TTL) to keep the database active, running more often than weekly (Upstash's Terms of Service allow deleting a free-tier resource after just one week without requests, so a weekly heartbeat has no safety margin against a missed or delayed run) so every run lands with real headroom, for example every 3 days; if that ever proves unreliable, the fallback is a paid Upstash plan, which extends the inactivity window rather than changing any eviction setting; document the chosen mitigation in this file's CHANGELOG entry when done | Verify: the heartbeat workflow runs green on schedule and the Upstash dashboard shows recent activity on garam-alert-dedupe
