...
### PR triage salvage queue (2026-09-11, from flagged-to-close PRs)

- [ ] ENHANCEMENT: [PRODUCER-SIGNUP-EMAILS] Email the owner whenever someone joins the Spice List or signs a waiver, and record failed signups in analytics so silent losses become visible | Why: flagged-to-close PR #230 built this (reportClaimFailure.ts, producer notifications in capture-lead/stage-waiver, trackError in the signup modals) but never merged and nothing else landed it | Files: src/components/reportClaimFailure.ts, src/pages/api/capture-lead.ts, src/pages/api/stage-waiver.ts, src/components/LeadCaptureModal.astro, src/components/NotifyModal.astro, src/components/HomeSignup.astro | Plan: recreate PR #230 on fresh main | Verify: signup and waiver flows fire a producer email in the test harness and a forced failure records an analytics error event.
- [ ] ENHANCEMENT: [WELCOME-EMAIL-COPY] Swap in the owner's rewritten applicant welcome email (live show volunteer pitch plus tickets link) | Why: flagged-to-close PR #236 carried the owner's verbatim wording in src/data/email-copy/*.json plus an emails.ts refactor; applicants still get the old email | Files: src/data/email-copy/, src/lib/emails.ts | Plan: recreate PR #236 on fresh main, copying the owner's wording byte-for-byte from the old PR (verbatim user copy is immutable, never paraphrase) | Verify: rendered welcome email matches the PR #236 wording exactly and email tests pass.
- [ ] ENHANCEMENT: [COMPLETED-SHOW-SYNC-WINDOW] Stop re-checking ticket sales for shows that already happened, with the cutoff computed in each show's own timezone | Why: the one unlanded piece of flagged-to-close PR #237 (its redirect fix and label correction merged via #235 and #249) | Files: src/pages/api/sync-orders.ts, src/lib/dateInZone.ts, src/pages/api/_tests/sync-orders.test.ts | Plan: recreate the date-window guard and dateInZone helper from PR #237 on fresh main | Verify: sync-orders tests cover a past show skipped and a future show synced across timezones.
- [ ] ENHANCEMENT: [JOURNAL-CUPID-ART] Place the unused cupid-garden illustration on the journal page | Why: the artwork half of flagged-to-close PR #146; public/images/ai-art/cupid-garden.webp has sat unused since April and the journal page never references it | Files: src/pages/journal/index.astro | Plan: one small change adding the illustration to the journal page layout | Verify: journal page renders the image, build passes.
- [ ] ENHANCEMENT: [PORTAL-FAST-LOAD] Contestant portal pre-hydration load, zero layout shift and design pass | Why: flagged-to-close PR #147 built this (portalBootstrap.ts, ContestantPortal.module.css, reserved loading layout, invite-link referrer hygiene) and none of it landed; the portal still fetches in useEffect and renders an unreserved Loading div | Files: src/lib/portalBootstrap.ts, src/components/ContestantPortal.tsx, src/components/ContestantPortal.module.css, src/pages/contestant-portal.astro | Plan: recreate PR #147 on fresh main using its write-up as the spec, on top of the portal fixes that merged since (#132 timeouts, #140 clickwrap) | Verify: portal tests pass, no layout shift on load, data fetch starts before hydration.
- [ ] ENHANCEMENT: [WAITLIST-SENDER-TOOL] One-to-one Gmail waitlist sender so show announcements land in main inboxes instead of Promotions | Why: flagged-to-close PR #168 built scripts/waitlist/ as a local-only tool; it never merged and nothing replaced it; reusable for any future show announcement | Files: scripts/waitlist/, package.json | Plan: recreate PR #168 on fresh main, carrying over its setup instructions | Verify: dry-run mode lists recipients without sending and the send path is rate-limited one recipient at a time.
