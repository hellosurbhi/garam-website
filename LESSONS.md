...
**Then the same trap sprang again on the other half of the pattern, one review round later:** fixing the local part meant rewriting the whole regex, and the domain half came back as `[^\s@.]+(?:\.[^\s@.]+)+`, one dot between labels, while the validator's `[^\s@]+\.[^\s@]+` allows dots anywhere but the first and last character. So `priya@exa..mple.com` sailed past a pattern whose own comment said it mirrored the validator. Two lessons in one. First, "mirrors X" written in a comment is a claim, not a mechanism: a second hand-written spelling of someone else's grammar is a copy that drifts, and this one drifted inside the very commit that added the comment. Assert the relation instead, in a test that runs the candidates through the real validator and then through the redactor, so the coverage claim fails out loud when the front door widens. Second, when a pattern has to be a superset of another, get the superset by construction rather than by restating it: matching the whole run and asserting the required character in a lookahead cannot be narrower than the thing it wraps, whereas re-describing the structure can always be narrower in a way that reads correct.
