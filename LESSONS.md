...
## `ticketSource: "eventbrite-owned"` drifted stale for shows going forward, and the first fix wrongly rewrote past ones too

**What went wrong:** Two shows scheduled going forward (Philadelphia Aug 28 2026, Edison TBA) were labeled `ticketSource: "eventbrite-owned"`, the flag that gates whether the daily order-sync cron attempts to pull real purchase data and report it to Meta, when the business no longer controls an Eventbrite organizer account for either. Nothing in the code or logs surfaced this: the sync job is silent on success and only logs actual API errors, so a token with no real access to these events would either fail quietly or, worse, succeed against unrelated data, neither of which looks different from a healthy run in the logs. Separately, and worse: the first attempt at fixing this applied the "we don't control Eventbrite" fact to all 11 shows that had ever carried the flag, including 9 already-completed shows (Apr 19 through Aug 16 2026), overwriting a business-fact field on records of shows that had already happened based on a present-tense statement, without confirming the fact was true for each of those specific past shows individually. The owner does not have a reliable memory of which past shows really were on a business-owned account and which weren't, so re-asserting the old value with confidence was also not fully justified, just less disruptive.

**Why:** `ticketSource` is manually set, per-event business knowledge (see the field's own doc comment in `events.ts`), not derivable from any other field, so nothing in the codebase could ever catch it drifting out of sync with reality. The account-access statement that triggered this fix ("nothing is our Eventbrite anymore") was true only as a going-forward, present-tense fact about upcoming bookings. Applying it with `replace_all` across every entry that shared the old value conflated "true now" with "true always," overwriting entries whose value was a correct historical record, not a stale one.

**Rule:** A business-fact flag that gates a real external side effect (an API call to a third-party account, a paid ad platform signal) needs periodic confirmation from whoever owns that fact, not a one-time-set label trusted forever. But when the business owner corrects that fact, the correction is scoped to what changed, not to every record that happens to share the old value: check each entry's own date/status against whether the stated fact was true for it specifically before editing it, especially for anything representing a completed, historical event. `replace_all` across a shared literal is a signal to stop and check scope by hand, not a shortcut to skip that check.

## A stale `ticketSource` on an old show could silently stall order-sync for every current show too, via a shared cursor

**What went wrong:** Chasing the exact historical `ticketSource` value for 9 already-completed shows (above) turned out to matter for a bigger reason than "get the record straight." `sync-orders.ts` shares one `changed_since` sync cursor across every syncable event, and holds that cursor at its previous value for the whole run if even one syncable event's Eventbrite fetch fails, with no distinction between a transient blip and a permanent one. A single old show wrongly (or no longer verifiably) marked `eventbrite-owned`, one we don't actually have order access to, could fail its fetch every single day and permanently jam Purchase-tracking sync for every other show, including the ones that actually matter right now, with no alert anywhere in that path.

**Why:** The cursor is shared, global state, not per-event, so one event's permanent failure has blast radius over unrelated records. Nothing in the sync job distinguishes "this one event is broken" from "something is broken," so it defensively stalls everything rather than risk silently skipping a transient miss. Combined with `ticketSource` being an unverifiable, drift-undetectable business-fact field (see above), this made a fact the business owner could not even reliably confirm into something with a real, silent, compounding production consequence.

**Rule:** When a background job depends on a manually-set, undrift-detectable business fact AND shares mutable state (a cursor, a lock, a rate budget) across the records it processes, bound the blast radius structurally instead of relying on the fact staying correct forever. Here, that means excluding events past a trailing recency window from the syncable set regardless of `ticketSource`, since a show that old has no realistic new orders left to sync, so its label can never jam anything again. Reconfirming the fact from the business owner is not always a viable mitigation: they may not remember it either.
