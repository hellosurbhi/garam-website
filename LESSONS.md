...
## A smoke test that mocks Firebase but not the site's own API route can page production for real

**What went wrong:** The scheduled Smoke Tests apply happy-path test (`tests/smoke/critical-flows.spec.ts`) has been failing against production since at least 2026-07-08 (separate open bug). Every Wednesday that failure's catch block ran the real app code path, which calls `reportFailure()` to `/api/alert-failure`. The test mocked the three Firebase googleapis.com endpoints so no real Firebase account was needed, but never mocked `/api/alert-failure` because that route belongs to the site itself, not Firebase. So every weekly failure sent a genuine FAILURE email to production on-call with the test's fixture contact (`Smoke Tester` / `smoketest@example.com`), indistinguishable at a glance from a real applicant.

**Why:** The mental model was "mock every third-party network call so the test runs offline," which is correct for Firebase but incomplete: this site's own `/api/*` routes are same-origin and not caught by the "third-party" mental filter, yet they have real side effects (an email to a human) exactly like a third-party call would. A page.route() audit that lists "what's external" misses "what has a side effect," which is the property that actually matters for a smoke test running with `PLAYWRIGHT_BASE_URL=production`.

**Rule:** Before running any test against production, audit every route the code under test can call, not just the third-party ones, and mock any with a real-world side effect (sends an email, writes a record, charges a card). Grep the component's imports for the site's own alerting/notification helpers, not just "googleapis.com" or other obvious externals.
