...
**Rule:** Any integration where our code calls `preventDefault()` on a native fallback (a link, a form submit) in favor of a third-party JS SDK must pair that override with a check that verifies the expected DOM or behavior actually appeared, plus a recovery path that restores the native fallback on failure. A successful SDK init call is not proof the interaction will complete. What counts as "appeared" and how the recovery navigates are specific to the integration, so work them out per case rather than copying this one: for the Eventbrite modal it meant observing the appearance itself (a presence check at the deadline misread an opened-then-closed modal as a failure) and navigating in the same tab, because the deadline fired past the browser's transient activation window and `window.open` was popup-blocked there.
