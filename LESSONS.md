...
**The half-fix that reads as a whole one:** the first version of this change updated all three files to say both checks were REQUIRED and deleted the finding from `BUGS.md`, while the live ruleset still listed one. Documenting a gate is not enabling it, and a comment asserting a state nobody verified is the same false-success reporting this repo already banned in CI, just in prose. When the fix ends outside the repo, the file half says plainly what is not live yet, the backlog entry stays open with the exact click path, and it closes when a human confirms the merge box, not when the diff lands.
...
**The follow-on trap, caught in review the same day:** the first redaction pattern matched a conventional address (`[\w.+-]+@...`) and missed `priya!@example.com`, which `src/utils/validateEmail.ts` happily accepts, since it allows any non-space, non-`@` character in the local part and native browser validation is off. A redactor is not a validator and does not get to define what an address looks like: it has to cover everything the app LET IN, so it must be at least as permissive as the validator at the front door, and the two have to move together. Where the two disagree, err toward over-redaction. A package spec redacted by mistake costs one lookup in the alert email; a missed address is published.

**Then the same trap sprang again on the other half of the pattern, one review round later:** fixing the local part meant rewriting the whole regex, and the domain half came back as `[^\s@.]+(?:\.[^\s@.]+)+`, one dot between labels, while the validator's `[^\s@]+\.[^\s@]+` asks only for one dot with a character on either side of it somewhere in the domain and counts dots as ordinary characters everywhere else, so it accepts `priya@exa..mple.com`, `priya@.example.com` and `priya@example.com.` too. Those sailed past a pattern whose own comment said it mirrored the validator. "Mirrors X" written in a comment is a claim, not a mechanism: a second hand-written spelling of someone else's grammar is a copy that drifts, and this one drifted inside the very commit that added the comment.

**Third round, and the actual rule:** the fix for round two was a cleverer restatement of the grammar, so the third review asked the obvious question, what happens when the validator changes again. The answer was to stop restating. The redactor is now `[^\s@]+@[^\s@]+`, which describes no address at all: it takes the validator's shape and DROPS its one extra requirement rather than reproducing it, so it cannot be narrower than the front door no matter how the front door moves. When a pattern has to be a superset of another, subtract a requirement, never re-describe the structure: a re-description can always come out narrower in a way that reads correct, and it did, twice, in consecutive rounds. The test half has the same shape of fix. A table of addresses that pass the validator and then get redacted proves only that those addresses are covered, while the sentence above it claims coverage of the whole grammar, so the table is a false guarantee the moment the grammar moves. Sweep the input space instead: every string up to a length over an alphabet of the characters the grammar actually turns on (`@`, `.`, whitespace, one ordinary letter), plus every interesting character in every structural position, keeping whatever the real validator accepts and requiring it to disappear. A test that enumerates cannot go stale the way a test that remembers does.

**Fourth round, the blind spot the sweep could not express:** the enumerating sweep did what it promised for one address and could not see past it, because every candidate it built was a single address inside a whitespace-delimited wrapper. Two addresses next to each other is a different question, and the answer was wrong: greedy runs that cannot cross an `@` matched `priya@example.com,raj` and stopped, publishing `@other.example`. The second address was not redacted, it was beheaded, and a domain on a personal address still names the person. So the sweep generalised over the alphabet of one input and not over how inputs COMPOSE, and a redactor's input is a caught exception's message, where the separator is chosen by whoever threw. The rule that falls out: when a pattern runs repeatedly over free text, sweep the joins as well as the values, and let the composition sweep pick its separators from the same character set the value sweep already uses rather than from the one example in the bug report. The second half of it is that "nothing survived" has to be defined against the guarantee and not against a character: this redactor deliberately leaves a bare `@handle` alone, so "no `@` in the output" was the right assertion for one address and the wrong one for a pair, where a lone `@` between two redactions is expected and an `@` still attached to text is the leak.

## A prototype key is a hole in every allowlist keyed by a plain object

**What went wrong:** `test/required-checks.test.ts` exempts declared-advisory CI jobs from the assertions that make every `ci.yml` job a required check, and decided membership with `job.id in ADVISORY_JOBS` against a plain object. `in` walks the prototype chain, so `constructor`, `toString`, `valueOf` and `hasOwnProperty` were all "in" an empty allowlist. A job with one of those ids would have been exempt from every gating check without appearing in the list, which is exactly the undeclared advisory job the whole file exists to prevent.

**Why:** the hole was latent behind a stricter guard (the job list was pinned to exactly two ids), and opening it was a side effect of a correct fix in the round before, which relaxed that pin so adding a job would not fail with a message about the parser. Removing a guard exposes whatever the guard was accidentally covering, and nothing says the two are related in the diff.

**Rule:** an allowlist or a denylist whose keys come from parsed input is a `Map`, not an object literal. `Map` has no inherited keys on the reading side and no `__proto__` special case on the writing side, so it means exactly what it lists. Where the structure has to stay an object, use `Object.hasOwn`, never `in`. And decide membership in one named helper rather than at each call site, so the next reader cannot reintroduce the operator. When a guard is loosened, ask what else it was holding up: this bug shipped in the same commit that relaxed one, for good reasons that had nothing to do with it.

**The regression pin then re-broke what it was pinning, in the smaller way:** the test written for the fix above ran `constructor`, `toString`, `valueOf` and `hasOwnProperty` through the real parser and the real predicate and asserted the result was `[]`, hard-coded. That is true today, when the allowlist is empty, and it stays asserted after someone declares one of those four ids advisory with a reason and a matching CI job, which the file explicitly invites everyone else to do. So the test turned four job names into second-class ids and would have failed a correct entry with a message about `Object.prototype`, which is the same shape as the parser guard two rounds earlier that pinned the job list to exactly two ids and told the person adding a third that the parser was broken. The expectation is now read off `ADVISORY_JOBS` (`inherited.filter((id) => ADVISORY_JOBS.has(id))`), which is `[]` today, so the original hole stays pinned while a declared exemption is allowed to work. **Rule:** a regression test asserts the invariant against the source of truth, never a snapshot of what the source of truth currently says. "No prototype name is exempt" and "nothing is exempt unless listed" agree on an empty list and disagree on every non-empty one, and only the second is the property being defended. This is the redaction lesson's "stop restating" rule in the test half: restating a value the code already owns is a copy, and a copy drifts, whether it lives in a regex or in a `toEqual`.
