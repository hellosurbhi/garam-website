...
const otherImages = [
  {
    src: "src/data/Garammasaladating-12.jpg",
    name: "tickets-hero",
    dir: join(PUBLIC, "images"),
  },
  {
    src: "src/components/Garammasaladating-16.jpg",
    name: "testimonial-reaction",
    dir: join(PUBLIC, "images"),
  },
  {
    src: "src/components/Garammasaladating-1.jpg",
    name: "links-hero",
    dir: join(PUBLIC, "images"),
  },
  {
    src: "_U7A8276.jpg",
    name: "journal-featured",
    dir: join(PUBLIC, "images"),
  },
];
...
async function processImage(srcPath, outputDir, outputName, maxWidth = 1200) {
  ...
  log(
    `  ${srcPath} — ${meta.width}x${meta.height} (${(meta.size / 1024 / 1024).toFixed(1)}MB)`,
  );
  ...
  for (const ext of ["avif", "webp", "jpg"]) {
    ...
    log(
      `    → ${outputName}.${ext}: ${outMeta.width}x${outMeta.height} (${sizeKB}KB)`,
    );
  }
}
...
async function processOgImage() {
  ...
  log(
    `  → og-image.jpg: ${outMeta.width}x${outMeta.height} (${(outMeta.size / 1024).toFixed(0)}KB)`,
  );
}
