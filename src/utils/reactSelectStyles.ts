...
type SelectOptionBase = { value: string; label: string };
type BaseStyles<Option extends SelectOptionBase> = StylesConfig<
  Option,
  boolean,
  GroupBase<Option>
>;
...
export function adminSelectStyles<Option extends SelectOptionBase>(): BaseStyles<Option> {
  return {
...
export function formSelectStyles<Option extends SelectOptionBase>(): BaseStyles<Option> {
  return {
    control: (base: CSSObjectWithLabel, state) => ({
      ...base,
      borderRadius: "12px",
      border: `1px solid ${state.isFocused ? "#DC2626" : "rgba(0, 0, 0, 0.1)"}`,
    ...
    option: (base: CSSObjectWithLabel, state) => ({
      ...
      color: "var(--charcoal)",
      cursor: "pointer",
      "&:active": { background: "rgba(220, 38, 38, 0.2)" },
    }),
    placeholder: (base: CSSObjectWithLabel) => ({
      ...base,
